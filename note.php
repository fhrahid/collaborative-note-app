...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        // If category_id is 0, set it to NULL
        ...
        if (empty($errors)) {
            if ($is_edit) {
                ...
                $stmt = $db->prepare("UPDATE notes SET title = ?, content = ?, category_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?");
                $success = $stmt->execute([$title, $content, $category_id, $note['id'], $user_id]);
                $note_id = $note['id'];
                $message = 'Note updated successfully!';
            } else {
                // Create new note
                $stmt = $db->prepare("INSERT INTO notes (user_id, title, content, category_id) VALUES (?, ?, ?, ?)");
                $success = $stmt->execute([$user_id, $title, $content, $category_id]);
                        ...
                        <select id="category_id" name="category_id">
                            ...
                            <?php foreach ($user_categories as $category): ?>
                                <option value="<?php echo $category['id']; ?>" 
                                        <?php echo (($note['category_id'] ?? $_POST['category_id'] ?? '') == $category['id']) ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($category['name']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <small style="color: var(--text-secondary); margin-top: 8px; display: block;">
                            💡 <a href="categories.php" style="color: var(--border-focus); text-decoration: none;">Manage categories</a> to organize your notes better
                        </small>
                    </div>
